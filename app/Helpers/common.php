...
function initSeoData($item='', $type='home'){
    switch ($type) {
        ...
        case 'page':
            ...
            $data_seo = [
                'meta_title' => strip_quotes($item->meta_title),
                'meta_keyword' => !empty($meta_keyword) ? $meta_keyword : '',
                'meta_description' => strip_quotes($item->meta_description),
                'site_image' => $item->thumbnail,
...
function initGameBaiSidebar() {
    $data['game_bai'] = \App\Models\TopGameNew::with('post')->whereNotNull('home_feature')->whereNotIn('home_feature', [0])->orderBy('home_feature', 'ASC')->get();
...
function initRatingParam($rate, $count, $title = '', $url, $schema = false) {
    $data['avg'] = $rate;
    $data['count'] = $count;
    $data['title'] = $title;
    $data['schema'] = $schema;
...
function turnOnAjaxAmp(){
    ...
    header("Access-Control-Allow-Origin: https://".str_replace('.', '-', env('DOMAIN')).".cdn.ampproject.org");
...
function replace_img_attr_in_html($html, $new_attr_url = [], $boolean = false) {
    $doc = new \DOMDocument();
    ...
    {
        foreach ($tags as $tag) {
           foreach($new_attr_url as $attr){
                $tag->setAttribute($attr['attr'], $attr['value']);
           }
        }
...
function replaceSrcImg($html){
    ...
    foreach ($tags as $tag) {
        ...
        $new_src_url = env('APP_URL').$old_src;
