...
class RouterController extends WebController
{
    ...
    public function post($slug) {
        ...
        $oneItem->content = replaceSrcImg($oneItem->content);
        ...
        $data['limit'] = $limit  = 6;
        $data['related_post'] = Post::where(['status' => 1, 'category_id' => $oneItem->category_id, ['displayed_time', '<=', Post::raw('NOW()')], ['id', '<>', $oneItem->id]])->orderBy('displayed_time', 'DESC')->offset(($page-1)*6)->limit($limit)->get();
        ...
        if (!empty($oneParent))
            $breadCrumb[] = [
                'name' => $oneParent->sub_title,
                'item' => getUrlCate($oneParent),
                'schema' => false,
                'show' => true
            ];
        $breadCrumb[] = [
            'name' => $category->sub_title,
    ...
    private function convertContent($content, $extra_data = []) {
        ...
        if (!IS_AMP){
            ...
            $content = preg_replace('/(\<ul.*?)type\=[\'\"].*?[\'\"](.*?\>)/', '$1$2', $content);
        }
        return $content;
    }
    ...
    public function category($slugParent, $slug) {
        ...
        $data['post'] = Post::where(['status' => 1, 'category_id' => $oneItem->id, ['displayed_time', '<=', Post::raw('NOW()')]])
            ...
            ->limit($limit)
            ->get();
        $data['top_game'] = TopGameNew::with('post')->where('category_id', $oneItem->id)->orderBy('order_by', 'ASC')->get();
        
        $data['post_cate'] = Post::join('post_category', 'post_category.post_id', '=', 'post.id')->where('post.status', 1)->where('post_category.category_id', $oneItem->id)->orderBy('post.id', 'desc')->limit(10)->offset(($page-1)*5)->get();
