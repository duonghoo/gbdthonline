            ...
            <div class="col-12 col-lg-8 px-0 mb-5">
                ...
                <div class="d-flex justify-content-between mb-3 flex-column flex-lg-row">
                    <div class="d-flex align-items-center">
                        <a href="{{ getUrlCate($category) }}" class="text-black6 text-uppercase font-10 bg-grey7 py-1 px-2 category-tag text-decoration-none">{{ $category->sub_title }}</a>
                    </div>
                ...
                @if(!empty($optional->name))
                    <div class="d-flex flex-wrap border rounded shadow bg-white py-3 py-lg-0 px-0 px-lg-3 text-center mb-4 position-relative">
                        <div class="col-12 col-lg-9 d-flex mb-2 mt-0 my-lg-3 flex-column flex-lg-row">
                            <a href="{{ getUrlPost($oneItem) }}" class="d-lg-flex align-items-lg-center">
                    ...
                    <div class="pl-2 pl-lg-5 d-flex align-items-center">
                        <ul class="list-unstyled mb-0">
                            <li class="font-weight-bold">
                                <a href="{{getUrlAuthor($user)}}" class="font-weight-bold text-decoration-none text-black1" rel="nofollow">  {{ $user->author }}</a>
                            </li>
                ...
                <div id="fb-comment" class="my-3 bg-white">
                    <div class="fb-comments" data-href="{{ env('APP_URL') }}" data-width="100%" data-numposts="5" data-order-by="reverse_time"></div>
                </div>
                <?php else: ?>
                ...
                <section class="row mx-0 box-news-blue">
                    <h4 class="mb-4 w-100">
                        <a href="{{ getUrlLink('/') }}" class="text-white px-3 py-2 font-14 text-decoration-none d-inline-block">Bài viết liên quan</a>
                    </h4>
                        @if(IS_AMP)
                            @if(strpos(Request::root(), env('DOMAIN')) === false)
                                @php $src = '//'.env('DOMAIN') @endphp
                            @else
                                @php $src = url('/') @endphp
                            @endif
