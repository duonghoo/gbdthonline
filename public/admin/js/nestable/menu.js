$(document).ready(function () {
    if (typeof $('#nestable')[0] != 'undefined') {
        ...
        $('.category_select').click(function(){
            ...
            var rel = option.data('rel');
            var target = option.data('target');
            title = title.replace(/-/g, '');
            appendEditMenuItem(container, title, url,rel,target);
            updateOutput(container);
            toggleEditMenuItem();
            editMenuItem();
            deleteMenuItem();
...
function editMenuItem() {
    $('.apply_item').on('click',function () {
        ...
        var rel = container.find('.rel_item').first().val();
        var target = container.find('.target_item').first().val();
        container.data('name', name);
        container.data('url', url);
        container.data('rel', rel);
...
function appendEditMenuItem(container, title, url,rel,target) {
    var item = "<li class='dd-item' data-name='' data-url=''>\n" +
        ...
        "        <input type='text' class='form-control rel_item' value='' placeholder='rel'>\n" +
        "        <input type='text' class='form-control target_item' value='' placeholder='target'>\n" +
    ...
    $(item).find('.rel_item').val(rel);
    $(item).find('.target_item').val(target);
...
function listify(container, strarr) {
    ...
    $.each(obj, function(i, v) {
        if (typeof v == 'object') {
            ...
            var rel = v.rel;
            var target = v.target;
            var parent = appendEditMenuItem(container,name,url,rel,target);
            if (!!v.children){
